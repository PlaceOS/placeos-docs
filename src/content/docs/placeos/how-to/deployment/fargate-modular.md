...
Use the same `EnvironmentName` variable throughout the deployment process. _PlaceOS_ is the default but each deployment in the same VPC should configure its own `EnvironmentName`. The Stack name you choose for each component should be descriptive but has no effect on the function of the deployment.

### VPC Architecture: `vpc.yml` <a href="#vpc" id="vpc"></a>
...
### Security Groups: `sec_groups.yml` <a href="#security-groups" id="security-groups"></a>
...
### Elasticsearch (AWS Managed Service): `elasticsearch.yml` <a href="#elasticsearch" id="elasticsearch"></a>
...
### ElastiCache (AWS Managed Service): `elasticache-redis-cluster.yml` <a href="#elasticache" id="elasticache"></a>
...
### Application Load Balancer: `load-balancer-https.yaml` <a href="#alb" id="alb"></a>
...
### Elastic File System: `EFS.yml` <a href="#efs" id="efs"></a>
...
### Fargate Cluster: `ecs-cluster.yml` <a href="#cluster" id="cluster"></a>
...
### RethinkDB `rethinkdb-primary.yml` <a href="#rethinkdb" id="rethinkdb"></a>

[RethinkDB](https://github.com/rethinkdb/rethinkdb) serves as the database for PlaceOS. RethinkDB can operate as a cluster but for the purposes of this example you will deploy a single primary member.
...
### _etcd_: `etcd-service.yml` <a href="#etcd" id="etcd"></a>
...
### Dispatch: `dispatch-service.yml` <a href="#dispatch" id="dispatch"></a>
...
### NGINX: `nginx-service.yml` <a href="#nginx" id="nginx"></a>

[NGINX](https://github.com/nginxinc/) is the web server for PlaceOS. See more on our implementation [here](https://github.com/PlaceOS/nginx)
...
### Frontends: `frontends-service.yml` <a href="#frontends" id="frontends"></a>

[Frontends](https://github.com/PlaceOS/frontends) is a sidecar to the web server. It listens for published frontend repositories and clones them to the NGINX static folder on EFS.
...
### Auth: `auth-service.yml` <a href="#auth" id="auth"></a>
...
### Core: `core-service.yml` <a href="#core" id="core"></a>
...
### Triggers: `triggers-service.yml` <a href="#triggers" id="triggers"></a>
...
### Rubber Soul: `rubber-soul-service.yml` <a href="#rubber-soul" id="rubber-soul"></a>
...
### _init_: `init-service.yml` <a href="#init" id="init"></a>

[init](https://github.com/PlaceOS/init) initializes the PlaceOS instance and is the final step in the deployment. This service requires the **DNS Name** of the ALB as it's the URL for accessing the application. The email and password configured here will also create a login you can use once you deploy the application.
...
_`https://[Application_Load_Balancer_DNS_NAME]/login?continue=/backoffice`_
